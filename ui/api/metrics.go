...
func (mm *metricsMiddleware) Firmware(s ui.Session) ([]byte, error) {
	defer func(begin time.Time) {
		mm.counter.With("method", "view_firmware").Add(1)
		mm.latency.With("method", "view_firmware").Observe(time.Since(begin).Seconds())
	}(time.Now())
